...
{
    ALOGV("ADRIAN 32 %p", this);
    status_t a = writeAligned(val);
    ALOGV("ADRIAN 32aaa %p", this);
    return a;
}
...
status_t Parcel::writeAligned(T val) {
    ...
    ALOGV("ADRIAN Aligned 1");
    if ((mDataPos+sizeof(val)) <= mDataCapacity) {
restart_write:
    ALOGV("ADRIAN Aligned 2");
        *reinterpret_cast<T*>(mData+mDataPos) = val;
    ALOGV("ADRIAN Aligned 3");
        return finishWrite(sizeof(val));
    }

    ALOGV("ADRIAN Aligned 4");
    status_t err = growData(sizeof(val));
    ALOGV("ADRIAN Aligned 5");
    if (err == NO_ERROR) goto restart_write;
    ALOGV("ADRIAN Aligned 6");
    return err;
}
